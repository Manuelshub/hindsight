...
/**
 * Returns something that is not a `Decision` at all.
 *
 * The third control, and the one that proves a guarantee rather than a metric: a bug on the
 * far side of a `DecideFn` must be reported as a bad brain and never as bad usage. It runs
 * by default for the same reason the oracle does — a promise nobody exercises is a promise
 * that has already quietly stopped being true.
 */
function createBroken(): DecideFn {
  return async () => null as unknown as Decision;
}
...
export const BRAINS: Record<string, BrainSpec> = {
  ...
  broken: {
    kind: 'control',
    describe: 'returns null — contract control, must FAIL rather than crash the run',
    create: async () => createBroken(),
  },
  adapter: {
    kind: 'service',
